## Contenidos

* Plan de Pruebas(PP) para el proyecto **Hoptolt**. Este plan de pruebas define los objetivos, alcance y casos de prueba para evaluar las funcionalidades definidas en el SRS, y las historias de usuario(HU).
* Especificación de Casos de Prueba(ECP) para el proyecto **Hoptolt**. Este documento detalla de manera individual la ejecución y los resultados obtenidos para los casos de prueba definidos en el PP.
## Información de los ECS

### Plan de Pruebas(PP)

- **Código del ECS:** PP
- **Nombre del ECS:** Plan de Pruebas
- **Autor:** Kevin Asmal, Diego Delgado, Gabriel López,Marcelo Pareja
- **Proyecto:** Hoptolt
- **Localización:** ```C:\27835_G5_ADS\Biblioteca de Trabajo/1. ELICITACION/1.7 Pruebas Unitarias/```
- **Tipo de ECS:** Documento
- **Fecha de creación:** 21/01/2026
- **ID del proyecto:** 27835_G5_ADS

### Especificación de Casos de Prueba(ECP)

- **Código del ECS:** ECP
- **Nombre del ECS:** Especificación de Casos de Prueba
- **Autor:** Gabriel López,Marcelo Pareja
- **Proyecto:** Hoptolt
- **Localización:** ```C:\27835_G5_ADS\Biblioteca de Trabajo/1. ELICITACION/1.7 Pruebas Unitarias/```
- **Tipo de ECS:** Documento
- **Fecha de creación:** 25/01/2026
- **ID del proyecto:** 27835_G5_ADS

# Historial de Versiones

| Versión     | Fecha      | Responsable     |   Aprobado por                                                  |
| ----------- | ---------- | --------------- | ------------------------------------------------------------- |
| `Plan_Pruebas_HoptoltV1.0.pdf` | 21/01/2026 | Marcelo Pareja  | La versión fue aprobada por Gabriel López|
| `Plan_Pruebas_HoptoltV2.0.pdf` | 25/01/2026 | Marcelo Pareja  | La versión fue aprobada por Gabriel López|
| `Plan_Pruebas_HoptoltV3.0.pdf` | 05/02/2026 | Marcelo Pareja  | La versión fue aprobada por Gabriel López|
| `Casos_de_pruebaV1.0.pdf` | 25/01/2026 | Marcelo Pareja  | La versión fue aprobada por Gabriel López|
| `Casos_de_pruebaV2.0.pdf` | 05/02/2026 | Marcelo Pareja  | La versión fue aprobada por Gabriel López|